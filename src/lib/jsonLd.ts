...
type ArticleInput = { headline: string; description: string; path: string; dateModified?: string; image?: string };
...
export function createArticleJsonLd({ headline, description, path, dateModified, image }: ArticleInput) {
  const modified = dateModified ?? new Date().toISOString();
  return { '@context': 'https://schema.org', '@type': 'Article', headline, description, image: absoluteUrl(image ?? siteConfig.defaultImage ?? '/og-default.svg'), author: { '@type': 'Organization', name: siteConfig.authorName ?? siteConfig.name, url: siteConfig.authorUrl ?? siteConfig.siteUrl }, publisher: { '@type': 'Organization', name: siteConfig.name, url: siteConfig.siteUrl }, mainEntityOfPage: absoluteUrl(path), dateModified: modified };
}
